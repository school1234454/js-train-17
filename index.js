...
function checkWord(word, text) {
  ...
  let isWordPresent = regex.test(text);
...
function extractTextInParentheses(str) {
  ...
  let extractedTexts = matches.map((match) => match[1]);
...
function checkRegexMethods(regex) {
  ...
  if (regex.source.includes("(?:")) {
    usedMethods.push("non-capturing groups");
  }
  if (regex.source.includes("(?=")) {
    usedMethods.push("positive lookahead");
  }
  if (regex.source.includes("(?<=")) {
    usedMethods.push("positive lookbehind");
  }
  if (regex.source.includes("(?<")) {
    usedMethods.push("named capture groups");
  }
  return usedMethods;
